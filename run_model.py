...
class run_model:
    ...
    def run_epoch(self, epoch_number, sess, fp = None):
        ...
        #print ("Total Samples in Training:",self.dataset.datasets["train"].number_of_samples)
        steps_per_epoch = int(math.ceil(float(self.dataset.datasets["train"].number_of_samples) / float(self.config.batch_size)))
        #print ("Total steps per Epoch:",steps_per_epoch)
        ...
        for step in xrange(steps_per_epoch):
            ...
            if ((step  + 1) % self.config.config_dir["print_frequency"] == 0):

                # Print status to stdout.
                print('Step %d: loss = %.6f (%.3f sec)' % (step+1, loss_value, duration))
                sys.stdout.flush()

                # Evaluate against the training set.
                #print('\nTraining Data Eval:')
               # self.print_titles(sess, self.dataset.datasets["train"], 1)
                    
                # Evaluate against the validation set.
                #print('\nValidation Data Eval:')
                #self.print_titles(sess,self.dataset.datasets["valid"], 1)
    ...
    def print_titles(self, sess, data_set, total_examples):
        ...
        for i,states in enumerate(final_ds):
            ...
            # Get the index of the highest scoring symbol for each time step
            print ("Predicted Summary :" + self.dataset.decode_to_sentence(states))
            print ("True Summary:" + self.dataset.decode_to_sentence(true_labels[i]))
    ...
    def run_training(self):
        ...
        with tf.Graph().as_default():
            ...
            summary_writer = tf.summary.FileWriter(outdir + "Logs" ,graph=sess.graph)
